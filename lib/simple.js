...
/*exports.smsg = (conn, m, hasParent) => {
    ...
    m.reply = async (text, chatId, options) => {
    	let pp = await conn.profilePictureUrl(m.sender, 'image').catch(_ => 'https://cdn.btch.bz/file/73326768aee09a0576e0d.jpg')
        let { data } = await conn.getFile(await(await require('node-fetch')(pp)).buffer())
        conn.reply(chatId ? chatId : m.chat, text, m, { contextInfo: { mentionedJid: conn.parseMention(text), externalAdReply: { showAdAttribution: true, title: namebot, body: wm, sourceUrl: sourceUrl,  thumbnail: data }}, options })
    }
    m.name = m.pushName || conn.getName(m.sender)
